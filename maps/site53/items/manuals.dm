...
/obj/item/paper/sec_ctp
	...
	You do not have the right to either authorize or deny a test. Senior Researcher staff may test on Safe level SCPs at their discretion and do not need explicit approval. Euclid-Class SCPs require a signature, and stamp from the Research Director. Keter-Class SCPs require both Research Director and Site Director signatures, and stamps. Cross tests require both Research Director and Site Director signatures, and stamps along with O5 Command Approval. You may, however, deny a test depending on the site's security level, if the test is unethical, and or against regulations, or if the individuals are suspicious. <br>
	If a test conflicts with the SCP's containment procedures, you are to notify the Research Director. If the Research Director still approves the test, you may allow entry. Otherwise, the research staff must be held at the checkpoint until sufficient modifications, or permission are made.<br>
	...
	<b>STEP FOUR:</b> If the test is on a Euclid level SCP or higher, or the Researcher has a Class-D present with them, they are to be acompanied by Zone Security, one officer is for every 3 D-Class. Said Zone Officer is to guard the chamber as the test is conducted.<br>
