...
/datum/controller/subsystem/modpacks/Initialize()
	...
	for(var/datum/modpack/package as anything in all_modpacks)
		...
		if(QDELETED(package))
			log_debug("Modpack of type [package.type] is null or queued for deletion.")
		if(fail_msg)
			log_debug("Modpack [package.type] failed to pre-initialize: [fail_msg].")
		if(loaded_modpacks[package.name])
			log_debug("Attempted to register duplicate modpack name: [package.name].")
	...
	for(var/datum/modpack/package as anything in loaded_modpacks)
		...
		if(fail_msg)
			log_debug("Modpack [(istype(package) && package.name) || "Unknown"] failed to initialize: [fail_msg]")
	for(var/datum/modpack/package as anything in loaded_modpacks)
		var/fail_msg = package.post_initialize()
		if(fail_msg)
			log_debug("Modpack [(istype(package) && package.name) || "Unknown"] failed to post-initialize: [fail_msg]")
...
/datum/controller/subsystem/modpacks/tgui_static_data(mob/user)
	var/list/data = list()
...
// Логирование сломано. Увы.

// /datum/controller/subsystem/modpacks/tgui_state(mob/user)
