...
/datum/unit_test
	...
	/// Treat the unit test as a template if its type is the same as the value of this var
	var/template
	/// If we want to keep a unit test in the codebase but not run it for some reason.
	var/disabled = 0
	/// If the check can be left to do it's own thing, you must define a check_result() proc if you use this.
	var/async = 0
