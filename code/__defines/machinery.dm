...
// Used by firelocks
#define FIREDOOR_OPEN   (1<<0)
#define FIREDOOR_CLOSED (1<<1)
...
#define PART_STAT_INSTALLED  (1<<0)
#define PART_STAT_PROCESSING (1<<1)
#define PART_STAT_ACTIVE     (1<<2)
#define PART_STAT_CONNECTED  (1<<3)

// part_flags
#define PART_FLAG_LAZY_INIT   (1<<0) // Will defer init on stock parts until machine is destroyed or parts are otherwise queried.
#define PART_FLAG_QDEL        (1<<1) // Will delete on uninstall
#define PART_FLAG_HAND_REMOVE (1<<2) // Can be removed by hand
...
#define FAB_HACKED   (1<<0)
#define FAB_DISABLED (1<<1)
#define FAB_SHOCKED  (1<<2)
#define FAB_BUSY     (1<<3)
