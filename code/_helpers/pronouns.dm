...
/datum/proc/p_themself(capitalized, temp_gender)
	. = "itself"
	if(capitalized)
		. = capitalize(.)
...
/client/p_themself(capitalized, temp_gender)
	if(!temp_gender)
		temp_gender = gender
	. = "themselves"
	switch(temp_gender)
...
/mob/p_themself(capitalized, temp_gender)
	if(!temp_gender)
		temp_gender = gender
	. = "itself"
	switch(temp_gender)
...
/mob/living/carbon/human/p_themself(capitalized, temp_gender)
	var/skipface = (wear_mask?.flags_inv & HIDEFACE) || (head?.flags_inv & HIDEFACE)
	if(skipface)
		temp_gender = PLURAL
	return ..()
