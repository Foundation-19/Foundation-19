...
/datum/game_mode/meteor/process()
	...
	if(((world.time - SSticker.round_start_time) >= METEOR_FAILSAFE_THRESHOLD) && (meteor_severity < 15) && !failsafe_triggered)
		log_and_message_staff("Meteor mode severity failsafe triggered: Severity forced to 15.")
		meteor_severity = 15
	...
	if((world.time - SSticker.round_start_time) >= next_wave)
		...
		if(send_admin_broadcasts)
			log_and_message_staff("Meteor: Wave fired. Escalation: [escalated ? "Yes" : "No"]. Severity: [meteor_severity]/[maximal_severity]")
