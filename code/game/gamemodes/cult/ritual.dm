...
/obj/item/book/tome/examine(mob/user)
	...
	if(!iscultist(user))
		...
		to_chat(user, "The scriptures of the Scarlet King, The One Who Sees, The Geometer of Blood. Contains the details of every ritual his followers could think of. Most of these are useless, though.")
...
/mob/proc/make_rune(rune, cost = 5, tome_required = 0)
	...
	if(do_after(src, timer, bonus_percentage = 25))
		...
		log_and_message_staff("created \an [R.cultname] rune at \the [A.name].")
...
/mob/proc/cult_communicate()
	...
	log_and_message_staff("used a communicate verb to say '[input]'")
