...
/datum/antagonist/cultist/proc/add_ghost_magic(mob/observer/ghost/M)
	if(max_cult_rating >= CULT_GHOSTS_1)
		add_verb(M, TYPE_PROC_REF(/mob/observer/ghost, flick_lights))
		add_verb(M, TYPE_PROC_REF(/mob/observer/ghost, bloody_doodle))
		add_verb(M, TYPE_PROC_REF(/mob/observer/ghost, shatter_glass))
		add_verb(M, TYPE_PROC_REF(/mob/observer/ghost, slice))
		if(max_cult_rating >= CULT_GHOSTS_2)
			add_verb(M, TYPE_PROC_REF(/mob/observer/ghost, move_item))
			add_verb(M, TYPE_PROC_REF(/mob/observer/ghost, whisper_to_cultist))
			add_verb(M, TYPE_PROC_REF(/mob/observer/ghost, bite_someone))
			add_verb(M, TYPE_PROC_REF(/mob/observer/ghost, chill_someone))
			if(max_cult_rating >= CULT_GHOSTS_3)
				add_verb(M, TYPE_PROC_REF(/mob/observer/ghost, whisper_to_anyone))
				add_verb(M, TYPE_PROC_REF(/mob/observer/ghost, bloodless_doodle))
				add_verb(M, TYPE_PROC_REF(/mob/observer/ghost, toggle_visiblity))
...
/mob/observer/ghost/proc/shatter_glass()
	...
	playsound(loc, SFX_SHATTER, 50, 1)
...
/mob/observer/ghost/proc/slice()
	...
	playsound(loc, 'sounds/weapons/bladeslice.ogg', 50, 1)
