...
/datum/universal_state/nuclear_explosion/proc/play_cinematic_station_destroyed()
	sound_to(world, sound(/effects/explosionfar.ogg'))//makes no sense if you're not on the station but whatever
...
/datum/universal_state/nuclear_explosion/proc/play_cinematic_station_unaffected()
	cinematic.icon_state = "station_intact"
	sleep(5)
	sound_to(world, sound(/effects/explosionfar.ogg'))//makes no sense if you are on the station but whatever
