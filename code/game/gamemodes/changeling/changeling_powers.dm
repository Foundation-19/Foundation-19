...
/datum/power/changeling/regenerative_stasis/proc/make_ready(mob/living/user)
	if(user && state != 2)
		...
		sound_to(the_collective, sound(/effects/wind/spooky1.ogg'))
...
/datum/power/changeling/rapid_regeneration/proc/do_regen(mob/living/carbon/human/H)
	...
	H.adjustOxyLoss(-10)
	H.adjustFireLoss(-10)
	H.playsound_local(H, /effects/singlebeat.ogg', 50, FALSE, is_global = TRUE) // Audio feedback to highlight each regen tick
	if(active_ticks)
