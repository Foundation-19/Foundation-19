...
/datum/effect/effect/system/smoke_spread/chem/set_up(datum/reagents/carry = null, n = 10, c = 0, loca, direct)
	...
	if(show_log)
		if(carry.my_atom.fingerprintslast)
			...
			if(M)
				more = ADMIN_QUE(M)
			log_and_message_staff("A chemical smoke reaction has taken place in [ADMIN_VERBOSEJMP(location)] [contained]. Last associated key is [carry.my_atom.fingerprintslast][more].")
		else
			log_and_message_staff("A chemical smoke reaction has taken place in [ADMIN_VERBOSEJMP(location)]. No associated key.")
...
/datum/effect/effect/system/smoke_spread/chem/spores/spawnSmoke(turf/T, icon/I, smoke_duration, dist = 1)
	...
	spores.set_name("cloud of [seed.seed_name] [seed.seed_noun]")
