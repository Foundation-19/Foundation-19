		...
		if("01")
			...
			if(isWrench(W) && !anchored)
				...
				if(do_after(user, 5 SECONDS, src, bonus_percentage = 25, focus_sound = 'sounds/items/Ratchet.ogg'))
					...
					src.anchored = TRUE
					if(src.secure)
						src.("Secure Anchored Windoor Assembly")
					else
						src.("Anchored Windoor Assembly")
			...
			//Unwrenching an unsecure assembly un-anchors it. Step 4 undone
				...
				if(do_after(user, 5 SECONDS, src, bonus_percentage = 25, focus_sound = 'sounds/items/Ratchet.ogg'))
					...
					src.anchored = FALSE
					if(src.secure)
						src.("Secure Windoor Assembly")
					else
						src.("Windoor Assembly")
			...
			//Adding cable to the assembly. Step 5 complete.
				...
				if(do_after(user, 5 SECONDS, src, bonus_percentage = 25))
					if (CC.use(1))
						...
						src.state = "02"
						if(src.secure)
							src.("Secure Wired Windoor Assembly")
						else
							src.("Wired Windoor Assembly")
			else
				..()
		...
		if("02")
			...
			//Adding airlock electronics for access. Step 6 complete.
				...
				if(do_after(user, 5 SECONDS, src, bonus_percentage = 25, focus_sound = 'sounds/items/Screwdriver.ogg'))
					...
					src.("Near finished Windoor Assembly")
					src.electronics = W
				else
					W.dropInto(loc)
