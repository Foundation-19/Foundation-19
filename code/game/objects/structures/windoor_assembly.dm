	...
	switch(state)
		if("01")
			if(isWelder(W) && !anchored )
				...
				if (WT.remove_fuel(0,user))
					...
					if(do_after(user, 5 SECONDS, src, bonus_percentage = 25, focus_sound = 'sound/items/Welder2.ogg'))
			...
			if(isWrench(W) && !anchored)
				...
				if(do_after(user, 5 SECONDS, src, bonus_percentage = 25, focus_sound = 'sound/items/Ratchet.ogg'))
					if(!src) return
		...
		if("02")
			...
			if(isWirecutter(W) && !src.electronics)
				...
				if(do_after(user, 5 SECONDS, src, bonus_percentage = 25, focus_sound = 'sound/items/Wirecutter.ogg'))
					if(!src) return
			...
			//Adding airlock electronics for access. Step 6 complete.
				...
				if(do_after(user, 5 SECONDS, src, bonus_percentage = 25, focus_sound = 'sound/items/Screwdriver.ogg'))
					if(!src) return
			...
			//Crowbar to complete the assembly, Step 7 complete.
				...
				if(do_after(user, 5 SECONDS, src, bonus_percentage = 25, focus_sound = 'sound/items/Crowbar.ogg'))
