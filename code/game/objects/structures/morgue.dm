	...
	if(contents.len <= 0)
		...
		for(var/mob/living/M in contents)
				...
				for(var/I, I < 60, I++)
					...
					if(prob(40))
						...
						switch(desperation) //This is messy. A better solution would probably be to make more sounds, but...
							if(1)
								playsound(src.loc, /weapons/genhit.ogg', 45, 1)
								shake_animation(2)
								playsound(src.loc, /weapons/genhit.ogg', 45, 1)
							if(2)
								playsound(src.loc, /effects/grillehit.ogg', 45, 1)
								shake_animation(3)
								playsound(src.loc, /effects/grillehit.ogg', 45, 1)
							if(3)
								playsound(src, /effects/bang.ogg', 45, 1)
								if(prob(50))
									playsound(src, /effects/bang.ogg', 45, 1)
									shake_animation()
								else
									shake_animation(5)
							if(4)
								playsound(src, /effects/clang.ogg', 45, 1)
								shake_animation(5)
							if(5)
								playsound(src, /weapons/smash.ogg', 50, 1)
			...
			if(M.stat == DEAD)
				if(round_is_spooky())
					if(prob(50))
						playsound(src, /effects/ghost.ogg', 10, 5)
					else
						playsound(src, /effects/ghost2.ogg', 10, 5)
		...
		update()
		playsound(src.loc, /effects/spray.ogg', 50, 1)
	return
