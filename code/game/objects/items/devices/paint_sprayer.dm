...
/obj/item/device/paint_sprayer/on_update_icon()
	cut_overlays()
	add_overlay(overlay_image(icon, "paint_sprayer_color", paint_color))
	update_held_icon()
...
/obj/item/device/paint_sprayer/proc/apply_paint(atom/A, mob/user, params)
	...
	if (.)
		...
		playsound(get_turf(src), /effects/spray3.ogg', 30, 1, -6)
	return .
...
/obj/item/device/paint_sprayer/proc/change_color(new_color, mob/user)
	if (new_color)
		...
		update_icon()
		playsound(src, /weapons/flipblade.ogg', 30, 1)
		return TRUE
	return FALSE
