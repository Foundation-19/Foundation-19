...
/obj/item/material/knife/folding/attack_self(mob/user)
	...
	if(open)
		...
		playsound(user, /weapons/flipblade.ogg', 15, 1)
	else
...
/obj/item/material/knife/folding/on_update_icon()
	if(open)
		cut_overlays()
		add_overlay(overlay_image(icon, hardware_open, flags=RESET_COLOR))
		item_state = "knife"
	else
		cut_overlays()
		add_overlay(overlay_image(icon, hardware_closed, flags=RESET_COLOR))
