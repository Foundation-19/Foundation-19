...
/obj/item/grenade/chem_grenade/attackby(obj/item/W as obj, mob/user as mob)
	if(istype(W,/obj/item/device/assembly_holder) && (!stage || stage==1) && path != 2)
		...
		playsound(src.loc, /items/Screwdriver2.ogg', 25, -3)
		detonator = det
...
//				to_chat(user, SPAN_WARNING("You need to add at least one beaker before locking the assembly."))
			...
			playsound(src.loc, /items/Screwdriver.ogg', 25, -3)
...
/obj/item/grenade/chem_grenade/detonate(mob/living/user)
	...
	if(!has_reagents)
		icon_state = initial(icon_state) +"_locked"
		playsound(src.loc, /items/Screwdriver2.ogg', 50, 1)
	...
	playsound(src.loc, /effects/bamf.ogg', 50, 1)
