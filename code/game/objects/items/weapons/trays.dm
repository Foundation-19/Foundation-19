...
/obj/item/tray/Initialize()
	. = ..()
	create_reagents(0) // Create an empty reagents object so that trays work correctly in stoves

/obj/item/tray/InsertedContents()
	return carrying
...
/obj/item/tray/attackby(obj/item/W, mob/living/user)
	if(istype(W, /obj/item/material/kitchen/rollingpin) && user.a_intent == I_HURT)
		if(bash_cooldown < world.time)
			...
			playsound(user.loc, /effects/shieldbash.ogg', 50, 1)
...
/obj/item/tray/proc/can_add_item(obj/item/I)
	...
	return !I.anchored && I.canremove && \
		...
		!istype(I, /obj/item/tray) && calc_carry() + cost <= max_carry
...
/obj/item/tray/baking
	name = "baking sheet"
	desc = "A wide, flat metal sheet used for baking. You can put this into a stove to cook ingredients stored on it. It's too unwieldy to hold anything but food and drink."
	icon_state = "baking_sheet"
	w_class = ITEM_SIZE_LARGE
	max_carry = 4 * BASE_STORAGE_COST(ITEM_SIZE_NORMAL)

/obj/item/tray/baking/can_add_item(obj/item/I)
	return ..() && is_type_in_list(I, SScuisine.stove_accepts_items)
