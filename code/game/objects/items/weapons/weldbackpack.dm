...
/obj/item/weldpack/attackby(obj/item/W as obj, mob/user as mob)
	if(isWelder(W))
		...
		if(T.welding & prob(50))
			log_and_message_staff("triggered a fueltank explosion.", user)
...
/obj/item/scrubpack/attackby(obj/item/W, mob/user)
	...
	if (isScrewdriver(W))
		...
		playsound(src, /items/Screwdriver.ogg', 50, 1)
		return TRUE
...
/obj/item/scrubpack/proc/set_sound_state(on_off)
	if (on_off)
		sound_token = GLOB.sound_player.PlayLoopingSound(src, "\ref[src]", /machines/scrubber-active.ogg', 25, 3)
	else
