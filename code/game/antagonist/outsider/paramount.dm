...
/datum/antagonist/paramount/equip(mob/living/carbon/human/player)
	...
	player.equip_to_slot_or_store_or_drop(new /obj/item/clothing/head/helmet/space/psi_amp(player), slot_head)
	...
	player.equip_to_slot_or_store_or_drop(new /obj/item/clothing/under/psysuit(player), slot_w_uniform)
	player.equip_to_slot_or_store_or_drop(new /obj/item/clothing/suit/wizrobe/psypurple(player), slot_wear_suit)
	player.equip_to_slot_or_store_or_drop(new /obj/item/clothing/shoes/jackboots(player), slot_shoes)
	player.equip_to_slot_or_store_or_drop(new /obj/item/storage/backpack/satchel(player), slot_back)
	var/obj/item/clothing/gloves/color/gloves = new()
	gloves.color = COLOR_GRAY80
	player.equip_to_slot_or_store_or_drop(gloves, slot_gloves)
