...
/obj/machinery/newscaster/attackby(obj/item/I, mob/user)
	if (user.a_intent == I_HURT)
		...
		if (I.force < 15)
			...
			playsound(src, /effects/hit_on_shattered_glass.ogg', 100, 1)
			...
			playsound(src, /effects/Glassbr3.ogg', 100, 1)
			show_sound_effect(src.loc, soundicon = SFX_ICON_JAGGED)
		else
			visible_message(SPAN_DANGER("\The [user] smashes \the [src] with \an [I]!"))
			playsound(src, /effects/Glasshit.ogg', 100, 1)
			show_sound_effect(src.loc, soundicon = SFX_ICON_JAGGED)
			set_broken(TRUE)
			update_icon()
		return TRUE
