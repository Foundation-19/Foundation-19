...
/obj/machinery/suit_storage_unit/attack_hand(mob/user as mob)
	...
	if(panelopen) //The maintenance panel is open. Time for some shady stuff
		...
		dat+= text("<font color ='black'>A thick old-style button, with 2 grimy LED lights next to it. The [] LED is on.</font><BR><font color ='blue'><A href='?src=\ref[];togglesafeties=1'>Press button</a></font>",(safetieson? FONT_COLORED("green","<B>GREEN</B>") : FONT_COLORED("red","<B>RED</B>")),src)
...
/obj/machinery/suit_storage_unit/proc/toggleUV(mob/user as mob)
	if(!panelopen)
		...
		if(issuperUV)
			to_chat(user, SPAN_NOTICE("You slide the dial back towards \"185nm\"."))
			issuperUV = 0
		else
			to_chat(user, SPAN_WARNING("You crank the dial all the way up to \"15nm\"."))
			issuperUV = 1
		return
...
/obj/machinery/suit_storage_unit/proc/toggle_open(mob/user as mob)
	if(stat & NOPOWER)
		to_chat(user, SPAN_WARNING("The unit is offline."))
		return
	if(islocked || isUV)
...
/obj/machinery/suit_storage_unit/proc/start_UV(mob/user as mob)
	...
	if(stat & NOPOWER)
		...
		return
	if(occupant && safetieson)
		to_chat(user, SPAN_DANGER("WARNING:</span><span class='warning'> Biological entity detected in the confines of the Unit's storage. Cannot initiate cycle."))
		return
...
/obj/machinery/suit_storage_unit/attackby(obj/item/I as obj, mob/user as mob)
	if(isScrewdriver(I))
		...
		to_chat(user, text(SPAN_NOTICE("You [panelopen ? "open" : "close"] the unit's maintenance panel.")))
		updateUsrDialog()
		update_icon()
		return
	if(isCrowbar(I))
		if((stat & NOPOWER) && !islocked && !isopen)
			to_chat(user, SPAN_WARNING("You begin prying the unit open."))
			if(do_after(user, 50, src))
				isopen = 1
				to_chat(user, SPAN_WARNING("You pry the unit open."))
				update_icon()
		else if(islocked)
	...
	if( istype(I,/obj/item/clothing/shoes/magboots) )
		...
		if(boots)
			to_chat(user, SPAN_NOTICE("The unit already contains a pair of magboots."))
			return
	...
	if( istype(I,/obj/item/weapon/tank) )
		...
		if(tank)
			to_chat(user, SPAN_NOTICE("The unit already contains an air tank."))
			return
