...
/obj/machinery/porta_turret/proc/HasController()
	...
	return A?.turret_controls.len > 0
...
/obj/machinery/porta_turret/attackby(obj/item/I, mob/user)
	if(stat & BROKEN)
		...
		if(do_after(user, 50, src))
			...
			if(!anchored)
				playsound(loc, /items/Ratchet.ogg', 100, 1)
				anchored = TRUE
				update_icon()
	...
	switch(build_step)
		...
		if(1)
			if(istype(I, /obj/item/stack/material) && I.get_material_name() == MATERIAL_STEEL)
				...
				playsound(loc, /items/Ratchet.ogg', 75, 1)
		...
		if(2)
			if(istype(I, /obj/item/wrench))
				...
				playsound(loc, pick(/items/Welder.ogg',s/items/Welder2.ogg'), 50, 1)
				if(do_after(user, 20, src))
