...
/obj/machinery/self_destruct/attackby(obj/item/W as obj, mob/user as mob)
	if(isWelder(W))
		if(damaged)
			...
			if(do_after(usr, 12 SECONDS, src, bonus_percentage = 25))
	...
	if(istype(W, /obj/item/nuclear_cylinder))
		...
		if(do_after(user, 10 SECONDS, src, bonus_percentage = 25) && user.unEquip(W, src))
			cylinder = W
			density = TRUE
...
/obj/machinery/self_destruct/MouseDrop(atom/over)
	...
	if(over == usr && cylinder)
		if(armed)
			...
			if(do_after(usr, 10 SECONDS, src, bonus_percentage = 25))
