...
/atom/movable/screen/movable/ability_master/proc/update_spells(forced = 0)
	...
	for(var/atom/movable/screen/ability/changeling/C in ability_objects) // this entire function is unbearably bad
		C.update_icon()
...
/atom/movable/screen/ability/changeling
	background_base_state = "changeling"
	var/datum/power/changeling/ability
	var/icon/last_charged_icon
	var/icon/last_active_icon

/atom/movable/screen/ability/changeling/can_activate()
	return ability?.can_activate(usr)

/atom/movable/screen/ability/changeling/activate()
	if (can_activate())
		ability.pre_activate(usr)
	ability_master.update_spells(0) // Immediately update all icons to show the chem consumption

/atom/movable/screen/ability/changeling/on_update_icon()
	// A lot of this function is taken from /atom/movable/screen/ability/spell's update_charge
...
/atom/movable/screen/movable/ability_master/proc/add_changeling_power(datum/power/changeling/C)
	if (!C)
		return

	var/atom/movable/screen/ability/changeling/A = new()
	A.ability_master = src
	A.ability = C
