...
/datum/spell/targeted/harvest/cast(list/targets, mob/user)//because harvest is already a proc
	...
	for(var/obj/singularity/scarletking/large/N in narsie_list)
		destination = N.loc
		break
	if(destination)
		var/prey = 0
		...
		to_chat(user, SPAN_DANGER("...something's wrong!"))//There shouldn't be an instance of Harvesters when Nar-Sie (or the scarlet king) isn't in the world.
