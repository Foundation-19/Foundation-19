...
/obj/item/reagent_containers/glass/rag/proc/update_name()
	if(on_fire)
		set_name("burning [initial(name)]")
	else if(reagents.total_volume)
		set_name("damp [initial(name)]")
	else
		set_name("dry [initial(name)]")
...
/obj/item/reagent_containers/glass/rag/proc/remove_contents(mob/user, atom/trans_dest = null)
	...
	if(reagents.total_volume)
		...
		if(do_after(user, 0.66 SECONDS * reagents.total_volume, bonus_percentage = 25))
			if(trans_dest)
	...
	if(isliving(target))
		...
		if(on_fire)
			...
			if(user.zone_sel.selecting == BP_MOUTH)
				...
				var/grab_time = 8 SECONDS
				if (user.skill_check(SKILL_COMBAT, SKILL_TRAINED))
					grab_time = 4 SECONDS

				if (do_after(user, grab_time, target, do_flags = DO_DEFAULT | DO_SHOW_TARGET,bonus_percentage = 25))
					user.visible_message(
