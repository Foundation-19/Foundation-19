...
/atom/proc/add_hiddenprint(mob/M)
	...
	if (ishuman(M))
		...
		if (H.gloves)
			src.fingerprintshidden += "Station time: [station_time_timestamp()] (Wearing gloves). Real Time: [time_stamp()], Real name: [H.real_name], Key: [H.key]"
			return 0

	src.fingerprintshidden += "Station time: [station_time_timestamp()]. Real Time: [time_stamp()], Real name: [M.real_name], Key: [M.key]"
	return 1
...
/mob/examinate(atom/A as mob|obj|turf in view())
	...
	if(get_skill_value(SKILL_FORENSICS) >= SKILL_EXPERIENCED && get_dist(src, A) <= (get_skill_value(SKILL_FORENSICS) - SKILL_TRAINED))
		...
		if(clue && has_client_color(/datum/client_color/noir))
			playsound_local(null, pick(/effects/clue1.ogg's/effects/clue2.ogg'), 60, is_global = TRUE)
