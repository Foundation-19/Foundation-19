...
/obj/structure/kitchenspike/proc/try_spike(mob/living/target, mob/living/user)
	...
	if(suitable_for_butchery(target))
		...
		if(!do_after(user, 5 SECONDS, target, do_flags = DO_DEFAULT | DO_SHOW_TARGET, bonus_percentage = 25) || occupant || !target || QDELETED(target) || target.stat == CONSCIOUS || !target.Adjacent(user))
			return
...
/obj/structure/kitchenspike/mob_breakout(mob/living/escapee)
	...
	if(secures_occupant)
		...
		if(!do_after(escapee, 7 SECONDS, src, bonus_percentage = 25))
			return FALSE
...
/obj/structure/kitchenspike/proc/do_butchery_step(mob/user, next_state, butchery_string)
	...
	if(do_after(user, 5 SECONDS, src, bonus_percentage = 25) && !QDELETED(user) && !QDELETED(last_occupant) && occupant == last_occupant && occupant_state == last_state)
