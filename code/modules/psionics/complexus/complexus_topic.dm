...
/datum/psi_complexus/Topic(href, list/href_list)
	...
	if(!. && check_rights(R_ADMIN))
		if(href_list && href_list["remove_psionics"])
			if(!QDELETED(src) && owner && !QDELETED(owner.psi) && owner.psi == src)
				log_and_message_staff("removed all psionics from [key_name(owner)].")
		...
		if(href_list && href_list["trigger_psi_latencies"])
			log_and_message_staff("triggered psi latencies for [key_name(owner)].")
