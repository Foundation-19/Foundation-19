...
/datum/proc/tgui_status(mob/user, datum/tgui_state/state)
	...
	if(isobserver(user))
		...
		if(isghost(user) && user.can_admin_interact())
			. = max(., UI_INTERACTIVE)
	...
	var/result = state.can_use_tgui_topic(src_object, user)
	. = max(., result)
 ...
 * return UI_state The state of the UI.
 */
/datum/tgui_state/proc/can_use_tgui_topic(src_object, mob/user)
...
/mob/proc/shared_ui_interaction(src_object)
	...
	if(!client)
		return UI_CLOSE
	// Disable UIs if unconcious or UI is blocked.
