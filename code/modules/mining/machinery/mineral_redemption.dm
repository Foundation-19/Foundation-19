...
/obj/machinery/mineral/ore_redemption
	...
	desc = "A machine that accepts ore and instantly transforms it into workable material sheets. You'll get paid in cash for your work, if Logistics is doing their job right."
	...
	/// Details how many dollars each smelted ore is worth	// TODO: replace with cargo bounty system
	var/static/list/ore_values = list(
		MATERIAL_IRON = 1,
		MATERIAL_GLASS = 1,
		MATERIAL_SILVER = 16,
		MATERIAL_GOLD = 18,
		MATERIAL_TITANIUM = 30,
		MATERIAL_URANIUM = 30,
		MATERIAL_DIAMOND = 50
	)
...
/obj/machinery/mineral/ore_redemption/examine(mob/user)
	. = ..()
	if(panel_open)
		to_chat(user, SPAN_NOTICE("Alt-click to rotate the input and output direction."))

/// Turns ore into its refined type, and sends it to its material container
/obj/machinery/mineral/ore_redemption/proc/smelt_ore(obj/item/ore/gathered_ore)
...
/obj/machinery/mineral/ore_redemption/AltClick(mob/living/user)
	...
	if(!user.can_perform_action(src))
		return
	if(panel_open)
		input_turf = turn(input_turf, -90)
		ouput_turf = turn(ouput_turf, -90)
		to_chat(user, SPAN_NOTICE("You change [src]'s I/O settings, setting the input to [dir2text(input_turf)] and the output to [dir2text(ouput_turf)]."))
	...
	switch(action)
		...
		if("Release")
			if(!mat_container)
				...
				materials.eject_sheets(mat, sheets_to_remove, get_step(src, ouput_turf))
			return TRUE
		if("Smelt")
			if(!mat_container)
				return
...
/obj/machinery/mineral/ore_redemption/update_overlays()
	...
	if((stat & NOPOWER))
		return
	var/image/ore_input = image(icon='icons/obj/doors/airlocks/station/overlays.dmi', icon_state="unres_[input_turf]")
	var/image/ore_output = image(icon='icons/obj/doors/airlocks/station/overlays.dmi', icon_state="unres_[reverse_dir[input_turf]]")
	...
	ore_input.color = COLOR_BLUE
	ore_output.color = COLOR_RED
