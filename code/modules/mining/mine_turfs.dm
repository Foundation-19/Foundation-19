...
/turf/simulated/mineral/on_update_icon(update_neighbors)
	if(!istype(mineral))
		...
		set_name("[mineral.ore_name] deposit")
	...
	for(var/direction in GLOB.cardinal)
		...
		else if(isspaceturf(turf_to_check) || istype(turf_to_check,/turf/simulated/floor))
...
/turf/simulated/floor/asteroid/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(valid_tool)
		...
		playsound(user.loc, 'sounds/effects/rustle1.ogg', 50, 1)
