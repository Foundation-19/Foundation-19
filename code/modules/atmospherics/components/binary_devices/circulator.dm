...
/obj/machinery/atmospherics/binary/circulator/on_update_icon()
	...
	if (last_pressure_delta > 0 && recent_moles_transferred > 0)
		if (temperature_overlay)
			add_overlay(image('icons/obj/power.dmi', temperature_overlay))
		if (last_pressure_delta > 5*ONE_ATMOSPHERE)
			add_overlay(image('icons/obj/power.dmi', "circ-run"))
		else
			add_overlay(image('icons/obj/power.dmi', "circ-slow"))
	else
		add_overlay(image('icons/obj/power.dmi', "circ-off"))
