...
/obj/machinery/atmospherics/pipe/cap/sparker/proc/ignite()
	playsound(loc, /machines/click.ogg', 10, 1)
	if (cant_ignite())
		return
...
/obj/machinery/atmospherics/pipe/cap/sparker/on_update_icon()
	..()
	if (signaler)
		add_overlay(image('icons/atmos/pipe-sparker.dmi', "pipe-sparker-s"))
	if (disabled)
		add_overlay(image('icons/atmos/pipe-sparker.dmi', "pipe-sparker-d"))
	update_underlays()
