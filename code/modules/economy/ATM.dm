...
/obj/machinery/atm/Process()
	...
	for(var/obj/item/spacecash/S in src)
		S.dropInto(loc)
		if(prob(50))
			playsound(loc, /items/polaroid1.ogg', 50, 1)
		else
			playsound(loc, /items/polaroid2.ogg', 50, 1)
		break
		...
		switch(href_list["choice"])
					...
					if(!authenticated_account)
						...
						if(previous_account_number == tried_account_num && tried_pin)
							if(number_incorrect_tries >= max_pin_attempts)
								...
								playsound(src, /machines/buzz-two.ogg', 50, 1)
			...
			if("e_withdrawal")
				...
				if(amount <= 0)
					...
					if(authenticated_account.withdraw(amount, "Credit withdrawal", machine_id))
						playsound(src, /machines/chime.ogg', 50, 1)
			...
			if("balance_statement")
				if(authenticated_account)
					...
					playsound(loc, prob(50) ? /items/polaroid1.ogg' :s/items/polaroid2.ogg', 50, 1)
