...
/proc/admin_attack_log(var/mob/attacker, var/mob/victim, var/attacker_message, var/victim_message, var/admin_message)
	...
	if(attacker)
		...
		if(victim)
			attacker.attack_logs_ += text("\[[station_time_timestamp()]\] <font color='red'>[key_name(victim)] - [attacker_message] [intent] [zone_sel]</font>")
		else
			attacker.attack_logs_ += text("\[[station_time_timestamp()]\] <font color='red'>[attacker_message] [intent] [zone_sel]</font>")
		attacker.last_attacked_ = mob_repository.get_lite_mob(victim)
		attack_location = get_turf(attacker)
	if(victim)
		if(attacker)
			victim.attack_logs_ += text("\[[station_time_timestamp()]\] <font color='orange'>[key_name(attacker)] - [victim_message] [intent] [zone_sel]</font>")
		else
