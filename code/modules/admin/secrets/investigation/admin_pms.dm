...
/datum/admin_secret_item/investigation/admin_pms/execute(mob/user, filter)
	...
	for(var/datum/admin_privat_message/pm in admin_pm_repository.admin_pms_)
		...
		if(receiver)
			dat += "<tr><td>[pm.recieved_time]</td><td>[sender.key_name(FALSE)] <a href='?src=\ref[src];filter=[html_encode(sender.ckey)]'>F</a></td><td>[receiver.key_name(FALSE)] <a href='?src=\ref[src];filter=[receiver.ckey]'>F</a></td></tr>"
		else
			dat += "<tr><td>[pm.recieved_time]</td><td>[sender.key_name(FALSE)] <a href='?src=\ref[src];filter=[html_encode(sender.ckey)]'>F</a></td><td></td></tr>"
