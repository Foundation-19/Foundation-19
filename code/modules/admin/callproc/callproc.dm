...
/datum/callproc/proc/finalise()
	...
	if(hastarget)
		if(!target)
			to_chat(usr, "Your callproc target no longer exists.")
			return
		log_and_message_staff("[key_name(C)] called \the [target]'s [procname]() with [arguments.len ? "the arguments [list2params(arguments)]" : "no arguments"].", location = get_turf(target))
		if(arguments.len)
		...
		returnval = call(text2path(procname))(arglist(arguments))
