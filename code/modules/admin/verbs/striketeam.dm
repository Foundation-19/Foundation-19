...
/client/proc/strike_team()
	...
	if(GAME_STATE < RUNLEVEL_GAME)
		to_chat(usr, FONT_COLORED("red","The game hasn't started yet!"))
		return

	if(world.time < 6000)
		to_chat(usr, FONT_COLORED("red","There are [(6000-world.time)/10] seconds remaining before it may be called."))
		return
	...
	if(team.deployed)
		to_chat(usr, FONT_COLORED("red","Someone is already sending a team."))
		return
