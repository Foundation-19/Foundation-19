...
/client/proc/fix_next_move()
	...
	for(var/mob/M in world)
		...
		M.next_move = 1
		M.next_click = 0
	message_staff("[key_name_admin(largest_move_mob)] had the largest move delay with [largest_move_time] frames / [largest_move_time/10] seconds!", 1)
	message_staff("[key_name_admin(largest_click_mob)] had the largest click delay with [largest_click_time] frames / [largest_click_time/10] seconds!", 1)
	message_staff("world.time = [world.time]", 1)
...
/client/proc/reload_admins()
	...
	message_staff("[usr] manually reloaded admins")
	load_admins()
