...
/datum/admins/proc/force_weather_state()
	...
	var/use_state = input(usr, "Which state do you wish to use?", "Target State") as null|anything in decls_repository.get_decl(/decl/state/weather)
	if(!use_state || weather != (T.weather || SSweather.weather_by_z[T.z]))
		return
	weather.weather_system.set_state(use_state)
	var/decl/state/weather/weather_state = decls_repository.get_decl(use_state)
