...
/mob/living/proc/can_enter_vent(obj/machinery/atmospherics/components/ventcrawl_target, provide_feedback = TRUE)
	...
	if(stat)
		if(provide_feedback)
			balloon_alert(src, "must be conscious!")
		return
	if(HAS_TRAIT(src, TRAIT_IMMOBILIZED))
		if(provide_feedback)
			balloon_alert(src, "immobilized!")
		return
	if(HAS_TRAIT(src, TRAIT_HANDS_BLOCKED))
		if(provide_feedback)
			balloon_alert(src, "hands blocked!")
		return
	if(has_buckled_mobs())
		if(provide_feedback)
			balloon_alert(src, "unbuckle mobs first!")
		return
	if(buckled)
		if(provide_feedback)
			balloon_alert(src, "unbuckle self first!")
		return
	if(iscarbon(src) && required_nudity)
		if(length(get_equipped_items(include_pockets = TRUE)) || get_num_held_items())
			if(provide_feedback)
				balloon_alert(src, "drop items first!")
			return
	if(ventcrawl_target.welded)
		if(provide_feedback)
			balloon_alert(src, "vent welded!")
		return

	if(!(vent_movement & VENTCRAWL_ENTRANCE_ALLOWED))
		if(provide_feedback)
			balloon_alert(src, "vent blocked!")
		return
...
/mob/living/proc/handle_ventcrawl(obj/machinery/atmospherics/components/ventcrawl_target)
	...
	if(HAS_TRAIT(src, TRAIT_MOVE_VENTCRAWLING) && istype(loc, /obj/machinery/atmospherics) && movement_type & VENTCRAWLING)
		balloon_alert(src, "ventcrawling out...")
	...
	//Entrance here
		...
		if(vent_parent && (vent_parent.members.len || vent_parent.other_atmos_machines))
			...
			balloon_alert_to_viewers("ventcrawling in...")
			...
			balloon_alert(src, "vent not connected!")

/mob/living/carbon/slime/can_enter_vent(obj/machinery/atmospherics/components/ventcrawl_target, provide_feedback = TRUE)
	if(Victim)
		if(provide_feedback)
			balloon_alert(src, "can't ventcrawl while feeding!")
		return
	return ..()
