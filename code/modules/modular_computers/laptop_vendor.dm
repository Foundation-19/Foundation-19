...
/obj/machinery/lapvend/proc/fabricate_and_recalc_price(fabricate = 0)
	...
	if(devtype == 1) 		// Laptop, generally cheaper to make it accessible for most station roles
		...
		switch(dev_disk)
			if(1) // Basic(128GQ)
				if(fabricate)
					fabricated_laptop.hard_drive = new/obj/item/stock_parts/computer/storage/hard_drive(fabricated_laptop)
			if(2) // Upgraded(256GQ)
				if(fabricate)
					fabricated_laptop.hard_drive = new/obj/item/stock_parts/computer/storage/hard_drive/advanced(fabricated_laptop)
				total_price += 99
			if(3) // Advanced(512GQ)
				if(fabricate)
					fabricated_laptop.hard_drive = new/obj/item/stock_parts/computer/storage/hard_drive/super(fabricated_laptop)
				total_price += 299
		switch(dev_netcard)
		...
		switch(dev_disk)
			if(1) // Basic(32GQ)
				if(fabricate)
					fabricated_tablet.hard_drive = new/obj/item/stock_parts/computer/storage/hard_drive/micro(fabricated_tablet)
			if(2) // Upgraded(64GQ)
				if(fabricate)
					fabricated_tablet.hard_drive = new/obj/item/stock_parts/computer/storage/hard_drive/small(fabricated_tablet)
				total_price += 99
			if(3) // Advanced(128GQ)
				if(fabricate)
					fabricated_tablet.hard_drive = new/obj/item/stock_parts/computer/storage/hard_drive(fabricated_tablet)
				total_price += 299
		switch(dev_netcard)
			if(1) // Basic(Short-Range)
				if(fabricate)
