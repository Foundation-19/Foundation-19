...
1. Basic information
Program based computers will allow you to do multiple things from single computer. Each computer will have programs, with more being downloadable from SCiPnet (sitewide network with wireless coverage)
...
Files can be programs (datum/computer_file/program/ subtype) or data files (datum/computer_file/data/ subtypes). Program for sending files will be available that will allow transfer via SCiPnet.
SCiPnet coverage will be limited to station's Z level, but better network card (=more expensive and higher power use) will allow usage everywhere. Hard drives will have limited capacity for files
...
Consoles won't usually have integrated battery, but the possibility to install one will exist for critical applications. Consoles are considered hardwired into SCiPnet network which means they
...
Laptops will by default have internal battery to power them, and may be recharged with rechargers. However, laptops rely on wireless SCiPnet coverage. Laptop HDDs are also designed with power efficiency
...
Network Card: Connects to SCiPnet
...
4. SCiPnet
SCiPnet is sitewide network that allows users to download programs needed for their work. It will be possible to send any files to other active computers using relevant program (NTN Transfer).
SCiPnet is under jurisdiction of both Engineering and Research. Engineering is responsible for any repairs if necessary and research is responsible for monitoring. It is similar to PDA messaging.
Operation requires functional "SCiPnet Relay" which is by default placed on tcommsat. If the relay is damaged SCiPnet will be offline until it is replaced. Multiple relays bring extra redundancy,
if one is destroyed the second will take over. If all relays are gone it stops working, simple as that. SCiPnet may be altered via administration console available to Chief Science Officer. It is
...
and similar programs which for some reason require HTML UI. Most software will be highly dependent on SCiPnet to work as laptops are not physically connected to the station's network.
What i plan to add:
...
Basic - Software in this bundle is automagically preinstalled in every new computer
	...
	NTNRC Chat - SCiPnet Relay Chat client. Allows PDA-messaging style messaging for more than two users. Person which created the conversation is Host and has administrative privilegies (kicking, etc.)
...
Engineering - Requires "Engineering" access on ID card (ie. CE, Atmostech, Engineer)
	...
	EngiDB - Allows accessing SCiPnet information repository for information about engineering-related things.
...
Medical - Requires "Medbay" access on ID card (ie. CMO, Doctor,..)
	...
	MediDB - Allows accessing SCiPnet information repository for information about medical procedures
	ChemDB - Requires extra access: "Chemistry" - Downloads basic information about recipes from SCiPnet
...
Research - Requires "Research and Development" access on ID card (ie. RD, Roboticist, etc.)
	...
	NTNRC Administration Console - Allows administrative access to SCPRC. This includes bypassing any channel passwords and enabling "invisible" mode for spying on conversations. Requires extra access: "Chief Science Officer"
	NTNet Administration Console - Allows remote configuration of SCiPnet Relay - CAUTION: If SCiPnet is turned off it won't be possible to turn it on again from the computer, as operation requires SCiPnet to work! Requires extra access: "Chief Science Officer"
	NTNet Monitor - Allows monitoring of SCiPnet and it's various components, including simplified network logs and system status.
...
Security - Requires "Security" access on ID card (ie. HOS, Security officer, Detective)
	...
	LawDB - Allows accessing SCiPnet information repository for security information (corporate regulations)
...
*REDACTED* - Can be downloaded from SyndiCorp servers, only via emagged devices. These files are very large and limited to laptops/consoles only.
	SYSCRACK - Allows cracking of secure network terminals, such as SCiPnet administration. The sysadmin will probably notice this.
	SYSOVERRIDE - Allows hacking into any device connected to SCiPnet. User will notice this and may stop the hack by disconnecting from SCiPnet first. After hacking various options exist, such as stealing/deleting files.
	SYSKILL - Tricks SCiPnet to force-disconnect a device. The sysadmin will probably notice this.
	SYSDOS - Launches a Denial of Service attack on SCiPnet relay. Can DoS only one relay at once. Requires SCiPnet connection. After some time the relay crashes until attack stops. The sysadmin will probably notice this.
...
They will have access to DOWNLOAD (not run) all programs that exist on SCiPnet. They'll have fairly good amount of available programs, most of them being
administrative consoles and other very useful things. They'll also be able to monitor SCiPnet. There will probably be one or two job slots.

8. IDS
With addition of various antag programs, IDS(Intrusion Detection System) will be added to SCiPnet. This system can be turned on/off via administration console.
If enabled, this system automatically detects any abnormality and triggers a warning that's visible on the SCiPnet status screen, as well as generating a security log.
