...
/datum/computer_file/program/ntnetmonitor/tgui_act(action, list/params, datum/tgui/ui, datum/tgui_state/state)
	...
	switch(action)
		...
		if("toggleWireless")
			...
			var/response = tgui_alert(usr, "Really disable SCiPnet wireless? If your computer is connected wirelessly you won't be able to turn it back on! This will affect all connected wireless devices.", "SCiPnet shutdown", list("Yes", "No"))
