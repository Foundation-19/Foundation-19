...
/datum/computer_file/program/game
	...
	program_menu_icon = "gamepad"		// Name of an icon (as in, tgui icon). Used for the main menu and file downloader.
...
/datum/computer_file/program/game/proc/game_check(mob/user)
	...
	if(boss_hp <= 0)
		...
		playsound(computer.loc, 'sounds/arcade/win.ogg', 50, TRUE, extrarange = -3, falloff = 0.1)
		game_active = FALSE
		if(istype(computer))
		...
		playsound(computer.loc, 'sounds/arcade/lose.ogg', 50, TRUE, extrarange = -3, falloff = 0.1)
		game_active = FALSE
		if(istype(computer))
			computer.update_icon()
...
/datum/computer_file/program/game/proc/enemy_check(mob/user)
	...
	if(boss_mp <= 5)
		...
		playsound(computer.loc, 'sounds/arcade/steal.ogg', 50, TRUE, extrarange = -3, falloff = 0.1)
		player_mp -= boss_mpamt
		boss_mp += boss_mpamt
	else if(boss_mp > 5 && boss_hp <12)
		heads_up = "[boss_name] heals for [bossheal] health!"
		playsound(computer.loc, 'sounds/arcade/heal.ogg', 50, TRUE, extrarange = -3, falloff = 0.1)
		boss_hp += bossheal
		boss_mp -= boss_mpamt
	else
		heads_up = "[boss_name] attacks you for [boss_attackamt] damage!"
		playsound(computer.loc, 'sounds/arcade/hit.ogg', 50, TRUE, extrarange = -3, falloff = 0.1)
 ...
 */
/datum/computer_file/program/game/tgui_assets(mob/user)
	return list(
...
/datum/computer_file/program/game/tgui_act(action, list/params)
	...
	switch(action)
		...
		if("Recharge_Power")
			...
			playsound(computer.loc, 'sounds/arcade/mana.ogg', 50, TRUE, extrarange = -3, falloff = 0.1)
