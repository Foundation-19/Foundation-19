...
/obj/machinery/power/port_gen/pacman
	...
	*/
	...
	working_sound = 'sounds/machines/engine.ogg'
	...
	if(istype(O, /obj/item/reagent_containers/))
		...
		if(R.standard_pour_into(src,user))
			if(reagents.has_reagent("vodka"))
				...
				playsound(get_turf(src),'sounds/machines/synth_yes.ogg', 50, 0)
			else
				audible_message(SPAN_WARNING("[src] blips in disappointment"))
				playsound(get_turf(src), 'sounds/machines/synth_no.ogg', 50, 0)
		return
	..()
