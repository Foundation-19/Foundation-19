...
/obj/machinery/light_construct/attackby(obj/item/W as obj, mob/living/user)
	...
	if(isWrench(W))
		...
		switch(stage)
			if (LIGHT_STAGE_EMPTY)
				playsound(loc, /items/Ratchet.ogg', 50, TRUE)
				...
				playsound(loc, /items/Deconstruct.ogg', 75, TRUE)
				qdel(src)
				return
	...
	if(isWirecutter(W))
		...
		playsound(loc, /items/Wirecutter.ogg', 50, TRUE)
		return
	...
	if(istype(W, /obj/item/stack/cable_coil))
		...
		if (coil.use(1))
			...
			playsound(loc, /items/Deconstruct.ogg', 50, TRUE)
		return
	...
	if(isScrewdriver(W))
		if (stage == LIGHT_STAGE_WIRED)
			...
			playsound(loc, /items/Screwdriver.ogg', 50, TRUE)
...
/obj/machinery/light/proc/broken(skip_sound_and_sparks = 0)
	...
	if(!skip_sound_and_sparks)
		if(lightbulb && !(lightbulb.status == LIGHT_BROKEN))
			playsound(loc, /effects/Glasshit.ogg', 50, TRUE)
		if(on)
			s.set_up(3, 1, src)
			s.start()
...
/obj/item/light/tube
	...
	sound_on = /machines/lightson.ogg'
