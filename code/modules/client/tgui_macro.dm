...
/datum/tgui_macro/New(client/C, datum/preferences/P)
	...
	owner = C
	if(C)
		INVOKE_ASYNC(C, TYPE_PROC_REF(/client, set_macros))
	prefs = P

/datum/tgui_macro/tgui_data(mob/user)
	. = list()
	.["keybinds"] = prefs.key_bindings

/datum/tgui_macro/tgui_static_data(mob/user)
	. = list()
	.["glob_keybinds"] = GLOB.ui_data_keybindings

/datum/tgui_macro/tgui_state(mob/user)
	return GLOB.always_tgui_state

/datum/tgui_macro/tgui_status(mob/user, datum/tgui_state/state)
	. = ..()
...
/datum/tgui_macro/tgui_act(action, list/params, datum/tgui/ui, datum/tgui_state/state)
	. = ..()
	if(.)
		return
			...
			if(!params["key"])
				if(kbinds[old_key])
					...
					if(!length(kbinds[old_key]))
						kbinds -= old_key
				INVOKE_ASYNC(owner, TYPE_PROC_REF(/client, set_macros))
