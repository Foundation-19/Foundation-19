...
/decl/overmap_event_handler/proc/update_hazards(turf/T)//catch all updater
	...
	if(!active_hazards.len)
		...
		UnregisterSignal(T, COMSIG_EXITED)
	else
		hazard_by_turf |= T
		hazard_by_turf[T] = active_hazards
		RegisterSignal(T, COMSIG_ENTERED, TYPE_PROC_REF(/decl/overmap_event_handler, on_turf_entered))
		RegisterSignal(T, COMSIG_EXITED, TYPE_PROC_REF(/decl/overmap_event_handler, on_turf_exited))
