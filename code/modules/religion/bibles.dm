...
/obj/item/book/bible/examine(mob/user)
	. = ..()
	if(deity_name)
		to_chat(user, SPAN_NOTICE("This bible has been approved by [deity_name]."))
	if(user.mind?.holy_role)
		if(GLOB.chaplain_altars.len)
			to_chat(SPAN_NOTICE("[src] has an expansion pack to replace any broken Altar."))
		else
			to_chat(SPAN_NOTICE("[src] can be unpacked by hitting the floor of a holy area with it."))
...
/obj/item/book/bible/suicide_act(mob/living/user)
	user.visible_message(SPAN_BOLDNOTICE("[user] is offering [user.p_them()]self to [deity_name]! It looks like [user.p_theyre()] trying to commit suicide!"))
	return BRUTELOSS
