/datum/event/fake_virus/start()
	...
	for(var/i in 1 to rand(1, defacto_min))
		...
		if(prob(25))//1/4 odds to get a spooky message instead of coughing out loud
			addtimer(CALLBACK(GLOBAL_PROC, /proc/to_chat, onecoughman, SPAN_WARNING("[pick("Your head hurts.", "Your head pounds.")]")), rand(3 SECONDS, 15 SECONDS))
		else
			addtimer(CALLBACK(onecoughman, /mob.proc/emote, pick("cough", "sniff", "sneeze")), rand(3 SECONDS, 15 SECONDS))//deliver the message with a slightly randomized time interval so there arent multiple people coughing at the exact same time
