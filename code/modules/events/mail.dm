...
/datum/event/mail/announce()
	command_announcement.Announce("A batch of mail addressed to the staff of [location_name()] has arrived at the sorting office and will arrive on the next available supply shuttle.", "Foundation Logistics Department", zlevels = affecting_z)
...
/datum/event/mail/proc/spawn_mail()
	...
	gift_crate.set_name("mail crate")

	for(var/name in to_receive)
		var/obj/item/documents/letter = new()
		letter.set_name("letter to [name]")
		...
		parcel.set_name("normal-sized parcel (to [name])")
