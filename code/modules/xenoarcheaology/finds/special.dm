...
/obj/item/vampiric/Process()
	...
	if(world.time - last_eat > eat_interval)
		...
		if(B)
			...
			if(istype(B, /obj/effect/decal/cleanable/blood/drip))
				...
				charges += 1
				playsound(src.loc, /effects/splat.ogg', 50, 1, -3)
			qdel(B)
	...
	if(charges >= 3)
		if(prob(5))
			...
			playsound(src.loc, pick(/hallucinations/growl1.ogg's/hallucinations/growl2.oggds/hallucinations/growl3.ogg'), 50, 1, -3)

	if(charges >= 1)
		if(shadow_wights.len < 5 && prob(5))
			shadow_wights.Add(new /obj/effect/shadow_wight(src.loc))
			playsound(src.loc, /effects/ghost.ogg', 50, 1, -3)
			charges -= 0.1
...
/obj/item/vampiric/proc/bloodcall(mob/living/carbon/human/M)
	...
	if(istype(M))
		playsound(src.loc, pick(/hallucinations/wail.ogg's/hallucinations/veryfar_noise.oggds/hallucinations/far_noise.ogg'), 50, 1, -3)
		nearby_mobs.Add(M)
...
/obj/effect/shadow_wight/Process()
	if(loc)
		...
		if(M)
			playsound(src.loc, pick(/hallucinations/behind_you1.ogg',\
			/hallucinations/behind_you2.ogg',\
			/hallucinations/i_see_you1.ogg',\
			/hallucinations/i_see_you2.ogg',\
			/hallucinations/im_here1.ogg',\
			/hallucinations/im_here2.ogg',\
			/hallucinations/look_up1.ogg',\
			/hallucinations/look_up2.ogg',\
			/hallucinations/over_here1.ogg',\
			/hallucinations/over_here2.ogg',\
			/hallucinations/over_here3.ogg',\
			/hallucinations/turn_around1.ogg',\
			/hallucinations/turn_around2.ogg',\
			), 50, 1, -3)
