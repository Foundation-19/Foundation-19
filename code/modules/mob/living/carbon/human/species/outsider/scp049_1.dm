...
/datum/species/scp049_1/handle_death(mob/living/carbon/human/H)
...
/datum/species/scp049_1/proc/handle_death_infection(mob/living/carbon/human/H)
	var/list/victims = hearers(rand(1, 2), H)
	for(var/mob/living/carbon/human/M in victims)
		if (H == M || M.is_species(SPECIES_SCP049_1))
			continue
...
/datum/species/scp049_1/handle_npc(mob/living/carbon/human/H)
	...
	H.resting = 0
	H.lying = 0
	H.species.show_ssd = null
	H.species.show_coma = null
...
/datum/species/scp049_1/proc/handle_action(mob/living/carbon/human/H)
	...
	for(var/mob/living/M in hearers(H, 15))
		if ((ishuman(M) || istype(M, /mob/living/exosuit)) && !M.is_species(SPECIES_SCP049_1) && !M.is_species(SPECIES_DIONA)) //Don't attack fellow zombies, or diona
	...
	if (target)
		if (target.is_species(SPECIES_SCP049_1) || istype(target, /mob/living/carbon/human/scp049))
			target = null
			return
...
/mob/living/carbon/human/proc/scp049_zombify()
	if (!(species.name in GLOB.zombie_species) || is_species(SPECIES_DIONA) || is_species(SPECIES_SCP049_1) || isSynthetic())
		return

	if (mind)
		if (mind.special_role == SPECIES_SCP049_1)
			return
		mind.special_role = SPECIES_SCP049_1

	var/turf/T = get_turf(src)
	new /obj/effect/decal/cleanable/vomit(T)
	playsound(T, 'sound/effects/splat.ogg', 20, 1)

	addtimer(CALLBACK(src, .proc/scp049_transform_zombie), 20)

/mob/living/carbon/human/proc/scp049_transform_zombie()
	make_jittery(300)
	adjustBruteLoss(100)
...
/mob/living/carbon/consume()
	...
	for (var/mob/living/carbon/human/L in get_turf(src))
		if (L != src && (L.lying || L.stat == DEAD))
			if (L.is_species(SPECIES_SCP049_1))
	...
	if (do_after(src, 5 SECONDS, target, DO_DEFAULT, INCAPACITATION_KNOCKOUT))
		...
		if (target.is_species(SPECIES_SCP049_1)) //Just in case they turn whilst being eaten
			return
