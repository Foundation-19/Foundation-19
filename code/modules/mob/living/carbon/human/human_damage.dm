...
/mob/living/carbon/human/adjustSanityLoss(amount)
	if(status_flags & GODMODE)	return 0	//godmode
	if(should_have_organ(BP_BRAIN))
		var/obj/item/organ/internal/brain/sponge = internal_organs_by_name[BP_BRAIN]
		if(sponge)
			sponge.take_sanity_damage(amount)

/mob/living/carbon/human/setSanityLoss(amount)
	if(status_flags & GODMODE)	return 0	//godmode
	if(should_have_organ(BP_BRAIN))
		var/obj/item/organ/internal/brain/sponge = internal_organs_by_name[BP_BRAIN]
		if(sponge)
...
/mob/living/carbon/human/getSanityLoss()
	...
	if(should_have_organ(BP_BRAIN))
		...
		if(sponge)
			if(sponge.status & ORGAN_DEAD)
				return 0 // cant be insane if your dead
			else
				return sponge.insanity
		else
...
/mob/living/carbon/human/restore_all_organs(ignore_prosthetic_prefs)
	...
	remove_verb(src, TYPE_PROC_REF(/mob/living/carbon/human, undislocate))
...
/mob/living/carbon/human/apply_damage(damage = 0, damagetype = BRUTE, def_zone = null, damage_flags = 0, obj/used_weapon = null, armor_pen, silent = FALSE, obj/item/organ/external/given_organ = null)
	...
	if(damage > 15 && prob(damage*1.35) && organ.can_feel_pain())
