	...
	if((user in contents) && istype(user))
		if(user.last_special <= world.time)
			...
			if(I?.force)
				...
				playsound(user.loc, /effects/attackblob.ogg', 50, 1)
...
/mob/living/carbon/proc/help_shake_act(mob/living/carbon/M)
	if(!is_asystole())
		if (on_fire)
			playsound(src.loc, /weapons/thudswoosh.ogg', 50, 1, -1)
			if (M.on_fire)
...
/mob/living/carbon/slip(slipped_on, stun_duration = 8)
	...
	playsound(loc, /misc/slip.ogg', 50, 1, -3)
...
/mob/living/carbon/proc/set_internals(obj/item/tank/source, source_string)
	...
	if(!old_internal && internal)
		...
		playsound(src, /effects/internals.ogg', 50, 0)
	if(old_internal && !internal)
