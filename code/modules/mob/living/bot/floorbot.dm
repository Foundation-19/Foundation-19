...
/mob/living/bot/floorbot/UnarmedAttack(atom/A, proximity)
	...
	if(emagged && istype(A, /turf/simulated/floor))
		...
		if(F.flooring)
			...
			if(do_after(src, 6 SECONDS, F, bonus_percentage = 25))
				F.break_tile_to_plating()
				addTiles(1)
		else
			visible_message(SPAN_DANGER("[src] begins to tear through the floor!"))
			if(do_after(src, 18 SECONDS, F, bonus_percentage = 25)) // Extra time because this can and will kill.
		...
		update_icons()
		anchored = TRUE
		if(do_after(src, 2 SECONDS, bonus_percentage = 25))
			if(T)
		...
		if(M.get_material_name() == MATERIAL_STEEL)
			...
			anchored = TRUE
			update_icons()
			if(do_after(src, 5 SECONDS, bonus_percentage = 25))
				if(M)
					M.use(1)
					addTiles(4)
			anchored = FALSE
