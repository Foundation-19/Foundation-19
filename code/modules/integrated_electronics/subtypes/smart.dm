...
/obj/item/integrated_circuit/smart/advanced_pathfinder/do_work()
	...
	var/list/P = AStar(a_loc, locate(get_pin_data(IC_INPUT, 1), get_pin_data(IC_INPUT, 2), a_loc.z), /turf/proc/CardinalTurfsWithAccess, /turf/proc/Distance, 0, 200, adjacent_arg=idc, exclude=get_turf(get_pin_data_as_type(IC_INPUT, 3, /atom)))
