/obj/item/gun/projectile
	...
	var/load_sound = /weapons/guns/interaction/bullet_insert.ogg'
	...
	var/mag_insert_sound = /weapons/guns/interaction/pistol_magin.ogg'
	var/mag_remove_sound = /weapons/guns/interaction/pistol_magout.ogg'
	...
	if(!is_jammed && prob(jam_chance))
		...
		if(istype(user))
			if(prob(user.skill_fail_chance(SKILL_WEAPONS, 100, SKILL_MASTER)))
				...
				is_jammed = 0
				playsound(src.loc, /weapons/flipblade.ogg', 50, 1)
	if(is_jammed)
		return null
	//get the next casing
	if(loaded.len)
