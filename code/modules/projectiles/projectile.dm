...
/obj/item/projectile/proc/on_hit(atom/target, blocked = 0, def_zone = null)
	...
	L.adjust_eye_blur(eyeblur)
	L.adjust_stutter(stutter)

	L.apply_effects(stun = 0, weaken = weaken, paralyze = paralyze, agony = 0, blocked = blocked)
...
/proc/calculate_projectile_Angle_and_pixel_offsets(atom/source, atom/target, modifiers)
	...
	if(!istype(user) || !user?.client || !modifiers["screen-loc"])
