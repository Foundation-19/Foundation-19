...
/datum/wires/tgui_state(mob/user)
	return GLOB.physical_tgui_state
...
/datum/wires/tgui_act(action, list/params)
	...
	switch(action)
		...
		if("cut")
			...
			playsound(holder, /items/Wirecutter.ogg', 20, 1)
		...
		if("pulse")
			...
			playsound(holder, /items/multitool_pulse.ogg', 20, 1)
			show_sound_effect(holder.loc, soundicon = SFX_ICON_SMALL)
			pulse_color(color)
...
/datum/wires/proc/can_see_wire_info(mob/user)
	if(user.skill_check(SKILL_ELECTRICAL, SKILL_EXPERIENCED))
		return TRUE
	return FALSE
...
/datum/wires/proc/detach_assembly(color)
	...
	if(S && istype(S))
		...
		S.connected = null
		if(istype(user))
			//There is an item that sleeps and that causes errors
			spawn(-1)
				user.put_in_hands(S)
		else
