...
/datum/wires/contraband_detector/get_status()
	...
	. += "The contraband identification light is [(haspower || !CD.identifier_wire_cut) ? (((CD.identifier_wire_pulsed_until < world.time) && CD.identifier_wire_pulsed_until != 0) ? "blinking!" : "on.") : "off!"]"
	. += "The radio light is [(haspower && !CD.radio_wire_cut) ? "on." : "off!"]"
...
/datum/wires/contraband_detector/on_cut(wire, mend)
	...
	switch(wire)
		if(WIRE_MAIN_POWER1, WIRE_MAIN_POWER2)
			CD.power_wire_cut = !mend
			CD.update_icon()

		if(WIRE_CONTRADETECT_IDENTIFIER)
			CD.identifier_wire_cut = !mend
			CD.announce_tampering()

		if(WIRE_CONTRADETECT_ALARM)
			CD.radio_wire_cut = !mend
...
/datum/wires/contraband_detectort/on_pulse(wire)
	...
	switch(wire)
		if(WIRE_MAIN_POWER1, WIRE_MAIN_POWER1)
			CD.announce_tampering()
