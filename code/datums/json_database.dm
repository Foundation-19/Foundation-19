	...
	if (fexists(filepath))
		...
		if (isnull(cached_data))
			...
			if (fexists(backup_filepath))
				...
				crash_with("[scenario]. No backup could be found.")
				cached_data = list()
	else
		if (fexists(backup_filepath))
...
/datum/json_database/proc/load_backup(scenario)
	...
	if (isnull(backed_up_data))
		crash_with("[scenario]. Backup existed, but also did not have valid JSON.")
		cached_data = list()
	else
		crash_with("[scenario]. Backup existed and was used instead. The JSON file has been updated.")
